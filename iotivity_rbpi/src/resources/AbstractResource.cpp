...
AbstractResource::AbstractResource(string Uri, string Type) : resourceUri(Uri), resourceType(Type)
{
    ...
    resourceTypes.push_back("core.light"); // resource type name. In this case, it is light
    resourceInterfaces.push_back(DEFAULT_INTERFACE); // resource interface.

	resourceRepresentation.setUri(Uri);
	resourceRepresentation.setResourceTypes(resourceTypes);
	resourceRepresentation.setResourceInterfaces(resourceInterfaces);
	resourceRepresentation.setValue("state", "running");
	...
	//Register resource
	OCStackResult result = OCPlatform::registerResource(resourceHandle, resourceUri, resourceTypes[0],
			resourceInterfaces[0], cb, resourceFlag);
